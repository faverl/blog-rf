...
// Intenta obtener el usuario para debug
...
if (data.access_token) {
  try {
    ...
    // ignora errores de debug
  }
}

// Devuelve el token al popup del CMS y también un mensaje JSON de debug
const html = `<!doctype html><html><body><script>
  (function(){
    ...
    var u = ${JSON.stringify(login)};
    if (t) {
      // Formato clásico usado por Decap/Netlify CMS
      send("authorization:github:" + t);
      // Mensaje alternativo para depurar e integrar
      send({ type: "decap-auth", provider: "github", token: t, user: u });
    } else {
      send("authorization:github:error");
      send({ type: "decap-auth", provider: "github", error: true });
    }
    window.close();
  })();
</script></body></html>`
